...
// ── Team name generator ───────────────────────────────────────────────────────
const ADJ = ["Birdie","Bogey","Eagle","Shanked","Mulligan","Sand Trap","Amen Corner",
  "Plaid","Green Jacket","Azalea","Dogleg","Bunker","Scratch","Yippy","Fat Shot",
  "Gimme","Lip Out","Duffed","Provisional","Fore!","Back Nine","Mudball","Lucky"];
const NOUN = ["Bandits","Hackers","Duffers","Gang","Crew","Squad","Syndicate",
  "Brigade","Outlaws","Caddies","Greenies","Bogeymen","Crushers","Warriors",
  "Legends","Posse","Wolves","Sharks","Rippers","Dreamers","Underdogs"];
const genName = () =>
          ...
          <div key={entry.id} className="card mb-2 overflow-hidden">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-display text-[15px] tracking-wide truncate">
                  {entry.teamName || entry.name}
                </div>
                {entry.teamName && (
                  <div className="text-[10px] text-t3 truncate">{entry.name}</div>
                )}
...
function LoginGate({ onSuccess, onGuestMode }) {
        ...
        <div className="card overflow-hidden">
          ...
          {onGuestMode && (
            <div className="border-t border-border px-5 py-4 text-center">
              <div className="text-[11px] text-t3 mb-3">Not in the league?</div>
              <button className="btn-ghost w-full" onClick={onGuestMode}>
                ENTER AS GUEST →
              </button>
            </div>
          )}
...
function EnterPicksTab({ entries, notify }) {
  ...
  const [guestPin2, setGuestPin2] = useState("");
  const [guestPinErr, setGuestPinErr] = useState("");
  const [guestInfoDone, setGuestInfoDone] = useState(false);
  ...
  useEffect(() => {
    if (existingEntry) {
      ...
      setTeamName(existingEntry.teamName ?? "");
    } else {
      setPicks(emptyPicks());
      setTiebreaker("");
      setTeamName("");
  ...
  const submitGuest = async () => {
    if (!guestName.trim()) { setGuestPinErr("Please enter your name."); return; }
    if (entries.find(e => e.name.toLowerCase() === guestName.trim().toLowerCase())) {
      setGuestPinErr("That name is already taken."); return;
    }
    if (!allPicked) { notify("Pick one golfer from every tier", "error"); return; }
    if (tiebreaker === "" || tiebreaker == null) { notify("Enter your tiebreaker guess", "error"); return; }
    if (!guestPin || guestPin.length !== 4 || !/^\d{4}$/.test(guestPin)) {
      setGuestPinErr("Please enter a 4-digit PIN."); return;
    }
    if (guestPin !== guestPin2) { setGuestPinErr("PINs do not match."); return; }
    setSubmitting(true);
    try {
  ...
  // Gate: must be logged in OR in guest mode
  if (!activePlayer && !guestMode) {
    return <LoginGate onSuccess={() => {}} onGuestMode={() => setGuestMode(true)} />;
  }
  ...
  if (submitted) {
    const displayEntry = guestMode ? guestTeamName || guestName : (teamName || player?.name ?? "");
    return (
      <div className="max-w-sm mx-auto text-center py-10 fade-up">
        ...
        <div className="text-t2 text-sm mb-6">Good luck at Augusta, {displayEntry}.</div>
        <button
  ...
  // Guest info screen (step 1 for guests)
  if (guestMode && !guestInfoDone) {
    return (
      <div className="max-w-lg mx-auto fade-up">
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="text-[11px] text-t3 font-display tracking-wider">GUEST ENTRY</div>
            <div className="font-display text-[18px] tracking-wide text-gold">YOUR INFO</div>
          </div>
          <button className="btn-ghost btn-sm text-[11px]" onClick={() => setGuestMode(false)}>← BACK</button>
        </div>
        <div className="card p-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full"
            />
            <div className="text-[11px] text-t3 mt-1">Shows on leaderboard instead of your name.</div>
          </div>
          <div>
            <div className="section-label mb-1">🔒 SET A PIN (to edit your name later)</div>
            <div className="flex gap-3">
              <div className="flex-1">
                <div className="text-[10px] text-t3 mb-1">PIN</div>
                <input
              ...
              <div className="flex-1">
                <div className="text-[10px] text-t3 mb-1">CONFIRM</div>
                <input
                  type="password" maxLength={4}
                  value={guestPin2}
                  onChange={e => setGuestPin2(e.target.value.replace(/\D/g, ""))}
                  placeholder="4 digits" className="w-full text-center"
                  style={{ letterSpacing: 6, fontSize: 18 }}
                />
              </div>
          ...
          {guestPinErr && <div className="text-[12px] text-red">{guestPinErr}</div>}
          <button
            ...
            onClick={() => {
              ...
              if (guestPin !== guestPin2) { setGuestPinErr("PINs do not match."); return; }
              setGuestPinErr("");
              setGuestInfoDone(true);
            }}
          >
            CONTINUE → PICK YOUR GOLFERS
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-lg mx-auto fade-up">
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        <div>
          <div className="text-[11px] text-t3 font-display tracking-wider">
            {guestMode ? "GUEST ENTRY" : isEditing ? "EDITING ENTRY" : "ENTERING AS"}
          </div>
          <div className="font-display text-[18px] tracking-wide text-gold">
            {guestMode ? (guestTeamName || guestName) : player?.name}
          </div>
        </div>
      ...
      {/* Team nickname field (league members only — guests set it earlier) */}
      {!guestMode && (
        <div className="card p-3 mb-4">
          <div className="flex items-center justify-between mb-1.5">
            <div className="section-label">TEAM NICKNAME (OPTIONAL)</div>
            <button
              className="text-[10px] text-gold border border-gold/40 rounded px-2 py-0.5 hover:bg-gold/10 transition-colors"
              onClick={() => setTeamName(genName())}
              type="button"
            >
              🎲 RANDOM
            </button>
          ...
          <input
            ...
            className="w-full"
          />
          <div className="text-[11px] text-t3 mt-1">Shown on leaderboard instead of your name.</div>
        </div>
      )}
      ...
      <div className="sticky bottom-20 md:bottom-4 pt-2 pb-1">
        <button
          ...
          onClick={guestMode ? submitGuest : submit}
...
function EditNameTab({ entries, notify }) {
  const { players } = usePlayers();
  ...
  const handleUnlock = async () => {
    ...
    if (pin === ADMIN_PIN) {
      setUnlocked(target);
      setNewName(target.teamName || "");
      setPin("");
      return;
    }
    // Guest entries have pinHash directly; league members verify against players list
    const hashToCheck = target.isGuest ? target.pinHash : players.find(p => p.id === target.playerId)?.pinHash;
    if (!hashToCheck) { setPinErr("Cannot verify PIN — ask the commissioner."); return; }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, "masters_entries", unlocked.id), { teamName: newName.trim() });
      notify("Team name updated! ✓");
      setUnlocked(null);
      setTarget(null);
      setSearch("");
    } catch (e) {
      notify("Failed to save — check connection.", "error");
    }
    setSaving(false);
  };
  ...
  if (unlocked) {
    return (
      <div className="max-w-sm mx-auto fade-up">
        <div className="font-display text-[11px] tracking-[2px] text-green mb-1">EDITING</div>
        <div className="text-[18px] font-semibold mb-4">{unlocked.name}</div>
        <div className="card p-5">
          <div className="flex items-center justify-between mb-1.5">
            <div className="section-label">TEAM NICKNAME</div>
            <button
              className="text-[10px] text-gold border border-gold/40 rounded px-2 py-0.5 hover:bg-gold/10 transition-colors"
              onClick={() => setNewName(genName())}
              type="button"
            >
              🎲 RANDOM
            </button>
          ...
          />
          <div className="text-[11px] text-t3 mb-4">Shown on leaderboard instead of your real name.</div>
          <div className="flex gap-3">
            <button className="btn-gold flex-1 py-2.5" onClick={handleSave} disabled={saving}>
              {saving ? "SAVING…" : "SAVE →"}
            </button>
            <button className="btn-ghost flex-1 py-2.5" onClick={() => { setUnlocked(null); setTarget(null); }}>
              CANCEL
            </button>
          </div>
        </div>
  ...
  if (target) {
    return (
      <div className="max-w-xs mx-auto text-center fade-up">
        ...
        <div className="text-[20px] font-semibold mb-1">{target.teamName || target.name}</div>
        {target.teamName && <div className="text-[12px] text-t3 mb-4">({target.name})</div>}
        <div className="card p-6">
          ...
          />
          {pinErr && <div className="text-[12px] text-red mb-3">{pinErr}</div>}
          <button className="btn-gold w-full py-2.5 mb-2" onClick={handleUnlock}>UNLOCK →</button>
          <button className="btn-ghost w-full text-[12px]" onClick={() => { setTarget(null); setPin(""); setPinErr(""); }}>← BACK</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="text-center mb-5">
        <div className="font-display text-[13px] tracking-[4px] text-green mb-1">MY NAME</div>
        <p className="text-[13px] text-t2">Find your entry and update your team nickname.</p>
      </div>
      <input
        value={search}
        onChange={e => setSearch(e.target.value)}
        placeholder="Search by name or team nickname…"
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map(entry => (
            <button
              ...
              onClick={() => { setTarget(entry); setPin(""); setPinErr(""); }}
            >
              <div>
                <div className="text-[16px] font-semibold">{entry.teamName || entry.name}</div>
                {entry.teamName && <div className="text-[11px] text-t3">{entry.name}</div>}
              </div>
...
export default function MastersPool({ notify }) {
  ...
  return (
    <div className="fade-up">
      ...
      {tab === "editname" && (
        <EditNameTab entries={entries} notify={notify} />
      )}
      {tab === "admin" && (
