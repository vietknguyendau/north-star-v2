...
const SEASON_EVENTS = [
  { id:"event-1", order:1, name:"Opening Classic",               date:"May 30, 2026",       course:"Rum River Hills",         multiplier:1 },
  { id:"event-2", order:2, name:"Early Summer Medal",            date:"June 20, 2026",       course:"Links at Northfork",      multiplier:1 },
  { id:"event-3", order:3, name:"Mid-Summer Medal",              date:"July 11, 2026",       course:"Oak Marsh / Eagle Valley",multiplier:1 },
  { id:"event-4", order:4, name:"North Star Mid-Season Major ⭐", date:"August 1, 2026",     course:"Keller / Edinburgh",      multiplier:2 },
  { id:"event-5", order:5, name:"Late-Season Push",              date:"August 22, 2026",     course:"Cedar Creek / Fox Hollow",multiplier:1 },
  { id:"event-6", order:6, name:"North Star Championship ⭐⭐",   date:"September 12, 2026", course:"Mystic Lake / Keller",     multiplier:3 },
];
...
.hist-event-card.oneoff { border-color:#2a1a3a; }
.hist-event-card.oneoff:hover { border-color:#7a4a9a; }
.hist-event-card.oneoff.active { border-color:#a06ad0; }
...
.oneoff-badge { font-family:'Bebas Neue'; font-size:9px; letter-spacing:1px; padding:1px 6px; border-radius:2px; background:#1a0a2a; color:#a06ad0; border:1px solid #4a2a6a; }
`;

export default function TournamentHistory({ players, adminUnlocked }) {
  const [snapshots,     setSnapshots]     = useState({});
  const [results,       setResults]       = useState({});
  const [eventMeta,     setEventMeta]     = useState({});
  const [pars,          setPars]          = useState([4,4,3,4,5,3,4,4,5,4,3,4,5,4,3,4,4,5]);
  const [oneOffTourneys,setOneOffTourneys] = useState([]);
  const [selectedEvent, setSelectedEvent] = useState(null);
  const [selectedType,  setSelectedType]  = useState("season"); // "season" | "oneoff"
  ...
  useEffect(() => {
    ...
    const u5 = onSnapshot(collection(db,"tournaments",TOURNAMENT_ID,"one_off_tournaments"), snap => {
  ...
  // Build one-off leaderboard from snapshot
  const buildOneOffLeaderboard = (tourney) => {
    if (!tourney.snapshot) return [];
    return [...tourney.snapshot].sort((a,b) => (a.net||999) - (b.net||999));
  };
  ...
  const selectedSeasonEvent = selectedType === "season" ? SEASON_EVENTS.find(e => e.id === selectedEvent) : null;
  const selectedOneOff      = selectedType === "oneoff" ? oneOffTourneys.find(t => t.id === selectedEvent) : null;
  const leaderboard         = selectedSeasonEvent ? buildLeaderboard(selectedSeasonEvent)
                            : selectedOneOff      ? buildOneOffLeaderboard(selectedOneOff) : [];
  const meta = selectedEvent && selectedType==="season" ? (eventMeta[selectedEvent] || {}) : {};
  const pot  = meta.pot || 0;

  const selectEvent = (id, type) => {
    setSelectedEvent(id); setSelectedType(type);
    setInnerTab("leaderboard"); setSelectedPlayer(null);
  };

  const renderScorecard = (leaderboard) => (
    <div>
          ...
          <div className="hist-card" style={{padding:"18px 20px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",gap:20}}>
                {[["GROSS",totalGross],["NET",totalNet],["HCP",p.handicap]].map(([lbl,val])=>(
  ...
  return (
    <div className="hist-wrap hist-fade">
      ...
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        <button className={`tab-pill ${mainTab==="season"?"active":""}`} onClick={()=>{ setMainTab("season"); setSelectedEvent(null); }}>
          🏌️ SEASON EVENTS
        </button>
        <button className={`tab-pill ${mainTab==="oneoff"?"active":""}`} onClick={()=>{ setMainTab("oneoff"); setSelectedEvent(null); }}>
          ⚡ ONE-OFF TOURNAMENTS
        </button>
      </div>
      ...
      {mainTab === "season" && (
        <>
          <div className="hist-label">── SEASON HISTORY · 2026</div>
          ...
          {selectedSeasonEvent && (
            <div className="hist-fade">
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{fontFamily:"'Bebas Neue'",fontSize:11,letterSpacing:3,color:"var(--green)",marginBottom:4}}>
                      EVENT {selectedSeasonEvent.order} · {selectedSeasonEvent.date}
                    </div>
                    <div style={{fontFamily:"'Bebas Neue'",fontSize:26,letterSpacing:2,color:"var(--text)"}}>
                      {selectedSeasonEvent.name.replace(" ⭐⭐","").replace(" ⭐","")}
                      {selectedSeasonEvent.multiplier===2&&<span className="mult-badge mult-2x" style={{marginLeft:8}}>2× POINTS</span>}
                      {selectedSeasonEvent.multiplier===3&&<span className="mult-badge mult-3x" style={{marginLeft:8}}>3× POINTS</span>}
                    </div>
                    <div style={{fontSize:13,color:"var(--text3)",marginTop:2}}>📍 {selectedSeasonEvent.course}</div>
                  </div>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                    {[["PLAYERS",leaderboard.length],["WINNER",leaderboard[0]?.name?.split(" ")[0]||"—"],pot?["POT",`$${pot}`]:null].filter(Boolean).map(([lbl,val])=>(
                      <div key={lbl} style={{textAlign:"center"}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
                {[["leaderboard","🏆 LEADERBOARD"],["scorecards","📋 SCORECARDS"],["points","⭐ POINTS"],["payouts","💰 PAYOUTS"]].map(([val,lbl])=>(
                  <button key={val} className={`tab-pill ${innerTab===val?"active":""}`} onClick={()=>{ setInnerTab(val); setSelectedPlayer(null); }}>{lbl}</button>
                ))}
              </div>
                ...
                <div className="hist-card" style={{overflow:"hidden",marginBottom:24}}>
                  ...
                  {leaderboard.map((p,idx)=>(
                    <div key={p.id} className="hist-row" style={{display:"grid",gridTemplateColumns:"44px 1fr 80px 80px 70px",padding:"12px 16px",alignItems:"center",
                      ...
                      <div style={{textAlign:"center",fontSize:12,color:"var(--text3)"}}>{p.scores?p.scores.filter(Boolean).length:"—"}/18</div>
                    </div>
                  ))}
                  {leaderboard.length===0&&<div style={{padding:"40px",textAlign:"center",color:"var(--text3)"}}>No results recorded.</div>}
                </div>
              )}
              {innerTab==="scorecards" && renderScorecard(leaderboard)}
              {innerTab==="points" && (
                <div className="hist-card" style={{overflow:"hidden",marginBottom:24}}>
                  ...
                  {leaderboard.map((p,idx)=>(
                    <div key={p.id} className="hist-row" style={{display:"grid",gridTemplateColumns:"44px 1fr 80px 80px",padding:"12px 16px",alignItems:"center",
                      ...
                      <div><div style={{fontSize:16,fontWeight:600,color:"var(--text2)"}}>{p.name}</div></div>
                      ...
                      <div style={{textAlign:"center"}}>
                        ...
                        {selectedSeasonEvent.multiplier>1&&<div style={{fontSize:9,color:"var(--text3)",marginTop:2,letterSpacing:1}}>{selectedSeasonEvent.multiplier}× MULTIPLIER</div>}
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    {(()=>{ const cfg=getPayouts(selectedEvent); return leaderboard.slice(0,cfg.splits.length).map((p,idx)=>(
                      <div key={p.id} className="hist-row" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:14}}>
                          <span className="place-badge" style={{color:idx===0?"var(--gold)":idx===1?"#90b0b8":idx===2?"#c08050":"var(--text2)"}}>{["1ST","2ND","3RD","4TH","5TH"][idx]}</span>
                          <div>
                            <div style={{fontSize:16,fontWeight:600,color:"var(--text2)"}}>{p.name}</div>
      ...
      {mainTab === "oneoff" && (
        <>
          ...
          {oneOffTourneys.length === 0 && !activeOneOff ? (
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12,marginBottom:28}}>
              {oneOffTourneys.map(t => {
                ...
                return (
                  <div key={t.id}
                    className={`hist-event-card oneoff ${isSelected?"active":""}`}
                    onClick={()=>selectEvent(t.id, "oneoff")}>
                    <div style={{padding:"14px 18px",borderBottom:"1px solid var(--border)",background:isSelected?"#1a0a2a":"var(--bg2)"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        <span className="oneoff-badge">ONE-OFF</span>
                        <span style={{fontSize:10,color:"var(--green)",fontFamily:"'Bebas Neue'",letterSpacing:1}}>✓ FINAL</span>
                      </div>
                      <div style={{fontSize:17,fontWeight:600,color:"var(--text)",marginTop:6}}>{t.title}</div>
                    </div>
                    <div style={{padding:"10px 18px",display:"flex",gap:16,fontSize:12,color:"var(--text3)"}}>
                      {t.date && <span>📅 {t.date}</span>}
                      {t.course && <span>📍 {t.course}</span>}
                      <span>👥 {(t.snapshot||[]).length} players</span>
                    </div>
                  </div>
                );
              })}
          ...
          {selectedOneOff && (
            <div className="hist-fade">
              <div style={{marginBottom:20,padding:"18px 22px",background:"var(--bg2)",border:"1px solid #4a2a6a",borderRadius:6,borderLeft:"3px solid #a06ad0"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{fontFamily:"'Bebas Neue'",fontSize:11,letterSpacing:3,color:"#a06ad0",marginBottom:4}}>
                      ONE-OFF TOURNAMENT{selectedOneOff.date ? ` · ${selectedOneOff.date}` : ""}
                    </div>
                    <div style={{fontFamily:"'Bebas Neue'",fontSize:26,letterSpacing:2,color:"var(--text)"}}>{selectedOneOff.title}</div>
                    {selectedOneOff.course && <div style={{fontSize:13,color:"var(--text3)",marginTop:2}}>📍 {selectedOneOff.course}</div>}
                    {selectedOneOff.notes && <div style={{fontSize:13,color:"var(--text3)",marginTop:4,fontStyle:"italic"}}>{selectedOneOff.notes}</div>}
                  </div>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                    {[["PLAYERS",leaderboard.length],["WINNER",leaderboard[0]?.name?.split(" ")[0]||"—"]].map(([lbl,val])=>(
                      <div key={lbl} style={{textAlign:"center"}}>
                        <div style={{fontFamily:"'Bebas Neue'",fontSize:22,color:"#a06ad0"}}>{val}</div>
                        <div style={{fontSize:9,color:"var(--text3)",letterSpacing:2,fontFamily:"'Bebas Neue'"}}>{lbl}</div>
                      </div>
                    ))}
                  </div>
              ...
              <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
                {[["leaderboard","🏆 LEADERBOARD"],["scorecards","📋 SCORECARDS"]].map(([val,lbl])=>(
                  <button key={val} className={`tab-pill ${innerTab===val?"active":""}`} onClick={()=>{ setInnerTab(val); setSelectedPlayer(null); }}>{lbl}</button>
                ))}
              </div>
              ...
              {innerTab==="leaderboard" && (
                <div className="hist-card" style={{overflow:"hidden",marginBottom:24}}>
                  ...
                  {leaderboard.map((p,idx)=>(
                    <div key={p.id} className="hist-row" style={{display:"grid",gridTemplateColumns:"44px 1fr 80px 80px 70px",padding:"12px 16px",alignItems:"center",
                      borderLeft:idx===0?"3px solid #a06ad0":"3px solid transparent"}}>
                      <span className="place-badge" style={{color:idx===0?"#a06ad0":idx===1?"#90b0b8":idx===2?"#c08050":"var(--text3)"}}>
                        {idx===0?"1ST":idx===1?"2ND":idx===2?"3RD":`${idx+1}`}
                      </span>
                      <div>
                        <div style={{fontSize:16,fontWeight:600,color:"var(--text2)"}}>{p.name}</div>
                      ...
                      <div style={{textAlign:"center",fontFamily:"'DM Mono'",fontSize:15,color:"var(--text2)"}}>{p.gross||"—"}</div>
                      <div style={{textAlign:"center",fontFamily:"'DM Mono'",fontSize:15,fontWeight:700,color:idx===0?"#a06ad0":"var(--text)"}}>{p.net||"—"}</div>
                      <div style={{textAlign:"center",fontSize:12,color:"var(--text3)"}}>{p.scores?p.scores.filter(Boolean).length:"—"}/18</div>
                    </div>
                  ))}
                  {leaderboard.length===0&&<div style={{padding:"40px",textAlign:"center",color:"var(--text3)"}}>No results recorded.</div>}
